...
void createBBallGraph(vector<BasketballPlayer> &bballers, unordered_map<string, int> &indHolderBBall, Graph &bballGraph)
{
    ...
    while (getline(file, line))
    {
        ...
        { // if we moved on to a different bball player, make sure we are adding the new player to the list and their attributes
        ...
        { // we have already visited this player, we are just editing its attributes
...
int main()
{
    ...
    cors
        .global()
        .headers("Content-Type", "X-Custom-Header", "Upgrade-Insecure-Requests", "Accept", "Access-Control-Allow-Origin") // TODO: Change X-Custom-Header; it' s placeholder
    ...
    CROW_ROUTE(app, "/bballAlgo")
        ...
        auto bfsTime = chrono::duration_cast<chrono::seconds>(stop - start);
    ...
    CROW_ROUTE(app, "/fballAlgo")
        ...
        auto stop = chrono::high_resolution_clock::now();
        auto bfsTime = chrono::duration_cast<chrono::milliseconds>(start - stop);
        start = chrono::high_resolution_clock::now();
